...
public class SincronismoWebTrans  {
	...
	public void Sincronizar()
	{
		...
		if (tipoperacao.contains("3"))	progress = ProgressDialog.show(context, "Aguarde..." , "Obtendo dados do WebTrans!!!",true,false);
		...
		new Thread( new Runnable() 
		{
			...
			public void run()                                                          
			{
				informUsr("Autenticando Talon�rio...");
					...
					try 
					{
						...
						else
						{
							...
							if (GetServiceClientes())
							{
								...
								clienteativo = "S";
								if ( j.getString("status").toUpperCase().contains("INATIVO")) clienteativo = "N";
			...
			//	usuarioWebTrans = "LOGWEBTRANS" + codMunicipio;
	            ...
	            // 16.05.2012
	            //
	            // nao consegui baixar do WebTrans ( exemplo n�o est� cadastrado agente corretamente )
	            // gera erro falta, sen�o n�o carrega agente e acaba pedindo o login sem ter agente cadastrado na
	            // tabela de agentes
	            //************************************************************************************
	            if (!lok)
	            {
	            		...
	            		while (tentar == true) {
							...
							handler.post(new Runnable() {
								...
								public void run() {
					        		...
					        		aviso.setNeutralButton("N�o",new DialogInterface.OnClickListener() {
	            ...
	            //1 - Inicializa��o 
	            //2 - Transmiss�o de aits e logs para o webtrans 
	            //3 - carga de tabelas
	            //*************************************************************************************************
	                             ...
	                             handler.post(new Runnable() {
									...
									public void run() {
										 AlertDialog.Builder aviso = new AlertDialog.Builder(context);
							        		aviso.setIcon(android.R.drawable.ic_dialog_alert);
							        		aviso.setTitle("Tabelas do WebTrans");
							        		aviso.setMessage("Falha ao baixar tabela de Esp�cies!\nDeseja tentar baixar novamente?");
							        		aviso.setNeutralButton("N�o",new DialogInterface.OnClickListener() {
							      ...
							      handler.post(new Runnable() {
										...
										public void run() {
											  AlertDialog.Builder aviso = new AlertDialog.Builder(context);
								        		aviso.setIcon(android.R.drawable.ic_dialog_alert);
								        		aviso.setTitle("Tabelas do WebTrans");
								        		aviso.setMessage("Falha ao baixar tabela de Pa�s!\nDeseja tentar baixar novamente?");
								        		aviso.setNeutralButton("N�o",new DialogInterface.OnClickListener() {
		            		...
		            		while (tentar == true) {
							    ...
							    handler.post(new Runnable() {
									...
									public void run() {
						        		...
						        		aviso.setIcon(android.R.drawable.ic_dialog_alert);
						        		aviso.setTitle("Tabelas do WebTrans");
						        		aviso.setMessage("Falha ao baixar tabela de Munic�pio!\nDeseja tentar baixar novamente?");
						        		aviso.setNeutralButton("N�o",new DialogInterface.OnClickListener() {
	            ...
	            {
		            //******************************************************************
		            // ocorreu algum erro fatal ?  nos JSONs ou na grava��o das tabelas  
		            //******************************************
	            	if (errofatal)
	            	{
		            	// fatal ?
			...
			//************************************************************************************************
			// ser� verifica antes de criar ait se existe arquivo de erro de ultima carga de dados do webtrans
			//************************************************************************************************
    ...
    public boolean trataEquipamentos(String tipx)
	{
		...
		return ret;
			...
			//************************************************************************************
			// Demais tratamentos geram erro fatal somente para operacao 1 ( inicializa��o do PDA ) 
	    	// ou quando n�o conseguem gravar ( tabela incopmleta )
			//*************************************************************************************
